...
class DB {
    ...
    async.waterfall([
      ...
      if(err) {
        if(err.code === 409) {
          ...
          debug('init', 'failed to connect to DB, trying in 2000ms')
  ...
  /**
   * Count the length of a collection
   *
   * @returns {Promise} which returns an integer(count)
   **/
  count(collection) {
    return new Promise((fulfill, reject) => {
      debug('count', collection)

      this.db.collection(collection).count()
      .then(data => {
        return fulfill(data);
      })
    })
